...
const goToReservation = async () => {
  ...
  const roomStr = (typeof sel === 'string') ? sel : (sel?.value ?? sel?.label ?? 'C1')

  // roomCode 매핑 (이미 정의된 로직과 동일)
  const mapRoomToCodeLocal = (roomName) => {
    if (!roomName) return 6
...
const boxMap = computed(() => {
  ...
  boxes.value.forEach((b, idx) => {
    // 서버가 준 라벨 우선 (예: "c5")
    const rawName = b.boxName ?? b.boxLabel ?? b.label ?? ''
    const name = String(rawName).trim()

    // 숫자 추출 (예: "c5" -> "5", "box12" -> "12")
    const digitMatch = name.match(/\d+/)
    const digit = digitMatch ? Number(digitMatch[0]) : null

    // finalLabel: 서버 라벨이 있으면 'bN' 형태로 정규화, 없으면 폴백으로 boxType+index 사용
    const prefix = (name && /[a-zA-Z]/.test(name[0])) ? name[0].toLowerCase() : boxType
    const finalLabel = (digit != null) ? `${prefix}${digit}` : `${boxType}${idx + 1}`

    // 여러 키로 등록 (소문자/대문자/숫자 키 호환성 유지)
    map[finalLabel] = b
    map[finalLabel.toLowerCase()] = b
    map[finalLabel.toUpperCase()] = b

    if (digit != null) {
      map[String(digit)] = b
      // 기존 호환: 접두사 조합 (a/A/b/B/c/C)
      prefixes.forEach(p => { map[p + digit] = b })
    }

    // 기존 numeric id 키도 유지
    const rawId = b.boxCode ?? b.box_id ?? b.id
    if (rawId != null) map[String(rawId)] = b
  })
...
watch(selectedRoom, (v) => {
  console.log('selectedRoom changed →', v, 'roomCode:', mapRoomToCode(v))
})
watch(boxMap, (newMap) => {
  console.log('boxMap updated for', selectedRoom.value, newMap)
})

// selectedRoom -> roomCode 변환
function mapRoomToCode(roomName) {
  if (!roomName) return 11
  const prefix = roomName[0].toUpperCase()
  const num = Number(roomName.slice(1)) || 1
  if (prefix === 'C') return 10 + num
}
...
async function fetchBoxesForRoom(roomName) {
  ...
  try {
    ...
    console.log('fetchBoxesForRoom', { roomName, roomCode })
